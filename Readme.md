...
1. If you're on Windows 11: ([source](https://github.com/ngo/win-frida-scripts/issues/2))
   - Disable LSA Protection:
      ...
      - Set `RunAsPPL` (DWORD) to `0`. This will disable the LSA Protection, [see this article for more information](https://learn.microsoft.com/windows-server/security/credentials-protection-and-management/configuring-additional-lsa-protection#disable-lsa-protection). Be aware that this lowers the security of your system, so only do this for the sake of keylogging Ubisoft Connect, then re-enable it afterwards.
      - Restart your computer
   - Disable Hardware-enforced Stack Protection for `lsass.exe`:
      - Then, open the `Windows Security` app
...
1. Start capturing keys with
    ...
    ```ps
    frida -l \path\to\keylog.js lsass.exe
    ```
...
1. In the filter bar, enter `(ip.dst_host == dmx.upc.ubisoft.akadns.net) || (ip.src_host == dmx.upc.ubisoft.akadns.net)`
